...
@property (nonatomic, weak) UIButton *editCodeBtn;
@end
...
/** 扫描动画线(冲击波) 的高度 */
static CGFloat const scanninglineHeight = 18;
/** 扫描内容外部View的alpha值 */
static CGFloat const scanBorderOutsideViewAlpha = 0.6;
...
#pragma mark - - - 扫描外部View的创建
    ...
    CGFloat light_buttonY = CGRectGetMaxY(promptLabel.frame) + 20;
    ...
    [light_button setTitleColor:[[UIColor greenColor] colorWithAlphaComponent:0.6] forState:(UIControlStateNormal)];
    [light_button setTitleColor:[[UIColor orangeColor] colorWithAlphaComponent:0.6] forState:(UIControlStateSelected)];
    ...
    // 添加手动输入
    UIButton *editCodeBtn = [[UIButton alloc] init];
    [editCodeBtn setImage:SetImage(@"editCode") forState:UIControlStateNormal];
    editCodeBtn.y = light_button.maxY + 20;
    editCodeBtn.size = editCodeBtn.currentImage.size;
    editCodeBtn.x = (self.width - editCodeBtn.width) * 0.5;
    [self addSubview:editCodeBtn];
    _editCodeBtn = editCodeBtn;
    
    // 添加手动输入文字描述
    UILabel *editCodeLabel = [[UILabel alloc] init];
    editCodeLabel.text = @"手动输入编号";
    editCodeLabel.textColor = [[UIColor whiteColor] colorWithAlphaComponent:0.6];
    editCodeLabel.textAlignment = NSTextAlignmentCenter;
    editCodeLabel.font = TextSystemFont(13);
    editCodeLabel.y = editCodeBtn.maxY + 10;
    editCodeLabel.x = 0;
    editCodeLabel.width = self.width;
    editCodeLabel.height = 15;
    [self addSubview:editCodeLabel];
...
#pragma mark - 手动输入编号点击事件
- (void)addTarget:(id)target action:(SEL)action {
    [self.editCodeBtn addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
}
