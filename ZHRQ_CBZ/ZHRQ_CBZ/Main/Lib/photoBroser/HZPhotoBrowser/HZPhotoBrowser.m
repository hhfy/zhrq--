...
- (void)longPressGesture:(UILongPressGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateBegan) {
        KNActionSheet *sheet = [[KNActionSheet alloc] initWithCancelTitle:@"取消" otherTitleArr:@[@"保存图片"] actionBlock:^(NSInteger buttonIndex) {
            if (buttonIndex == 0) {
                [self saveImage];
            }
        }];
...
- (void)addToolbars
{
    ...
    [saveButton setImage:SetImage(@"Download") forState:UIControlStateNormal];
